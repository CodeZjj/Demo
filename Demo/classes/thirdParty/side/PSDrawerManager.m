...
// 显示/隐藏侧边栏
- (void)showOrHideSideInView:(UIView *)view
{
    if (self.isShowSide) {
        [self hideSideInView:view];
    } else {
        [self showSideInView:view];
    }
    self.isShowSide = !self.isShowSide;
}

// 显示侧边栏
- (void)showSideInView:(UIView *)view
{
    [UIView animateWithDuration:0.2f animations:^{
        CGAffineTransform rightScopeTransform = CGAffineTransformTranslate([kAppDelegate window].transform, kScreenWidth * kLeftWidthScale, 0);
        view.transform = rightScopeTransform;
        //[kAppDelegate window].subviews.firstObject.tx = view.tx / 3;
    }];
}

// 隐藏侧边栏
- (void)hideSideInView:(UIView *)view
{
    [UIView animateWithDuration:0.2f animations:^{
